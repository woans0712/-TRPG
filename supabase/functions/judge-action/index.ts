...
serve(async (req) => {
  ...
  try {
    ...
    const system = [
      ...
      "'추천한다', '유리하다', '하는 것이 좋다' 같은 조언투를 쓰지 않는다.",
      "GM은 전략을 추천하지 말고, 행동의 결과와 눈앞의 장면만 묘사한다.",
      "이전 대화와 이벤트 로그를 우선해서 인과관계를 이어라.",
      "결과는 2~4문장으로 간결하게 쓴다.",
      "유저가 바로 이해할 수 있게 현재 상황, 행동의 결과, 열린 선택지를 압축해서 보여준다.",
    ...
    const verdict =
      (await openAIJson<VerdictPayload>(system, {
        ...
        action,
        judgment_style: {
          result_length: "2~4 Korean sentences",
          must_include: ["행동의 직접 결과", "이전 맥락과의 연결", "상황 변화", "새 단서 또는 다음 선택지"],
          avoid: ["아무 변화 없음", "반복 문장", "근거 없는 즉사", "플레이어 행동 무시", "추천한다", "유리하다", "하는 것이 좋다"],
        },
        schema: {

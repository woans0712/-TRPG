...
final class ChatExportParser {
    ...
    static List<ImportedMessage> parse(String text) {
        ...
        for (String rawLine : lines) {
            ...
            Matcher dash24 = DASH_24H_COMMA_MESSAGE.matcher(line);
            if (dash24.matches()) {
                last = buildFromDated24HourLine(dash24);
                messages.add(last);
                continue;
            }

            Matcher dot24 = DOT_24H_COMMA_MESSAGE.matcher(line);
            if (dot24.matches()) {
                last = buildFromDated24HourLine(dot24);
                messages.add(last);
                continue;
            }

            Matcher tab24 = TAB_24H_MESSAGE.matcher(line);
            if (tab24.matches()) {
                last = buildFromDated24HourLine(tab24);
                messages.add(last);
                continue;
            }
            ...
            Matcher bracket24 = BRACKET_24H_MESSAGE.matcher(line);
            if (bracket24.matches() && currentDate != null) {
                last = build24Hour(currentDate, bracket24.group(2), bracket24.group(3), bracket24.group(1), bracket24.group(4));
                messages.add(last);
                continue;
            }
            ...
            Matcher time24 = TIME_24H_COMMA_MESSAGE.matcher(line);
            if (time24.matches() && currentDate != null) {
                last = build24Hour(currentDate, time24.group(1), time24.group(2), time24.group(3), time24.group(4));
                messages.add(last);
                continue;
            }
    ...
    private static ImportedMessage build24Hour(LocalDate date, String hourText, String minuteText, String nickname, String message) {
        int hour = Integer.parseInt(hourText);
        int minute = Integer.parseInt(minuteText);
        LocalDateTime time = date.atTime(hour, minute);
        String occurredAt = DateTimeFormatter.ISO_INSTANT.format(time.atZone(KST).toInstant());
